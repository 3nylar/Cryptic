...
```python
import sys; sys.path.insert(0, "/path/to/cryptic")
...
Repeated 3-gram distances → factors → `{key_length: votes}`. Votes for the true length _and its factors_, which is why it is combined with the IC rather than trusted alone.
